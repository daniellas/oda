...
class ReportsGeneratorSpec extends FreeSpec {
  ...
  def generateJiraReports() = {
    Config.props.jira.projects.foreach(p => {
      Await.result(workItemsCountByState(p._1, ChronoUnit.DAYS), 10 minutes)
      Await.result(workItemsCountByStateMovingAverage(p._1, 14, ChronoUnit.DAYS), 10 minutes)
      //      Await.result(jiraCountByTypePriority(p._1, ChronoUnit.DAYS, p._2.stateMapping), 10 minutes)
